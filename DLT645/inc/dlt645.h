...
typedef struct dlt645
{
    uint8_t addr[6];    //从机地址
    uint8_t debug;      //调试标志
    int (*write)(SerialPort self, uint8_t *buf, uint16_t len);     //底层写函数
    int (*read) (SerialPort self, uint8_t *msg ,uint16_t len);		//底层读函数
} dlt645_t;
...
//数据采集
extern int dlt645_read_data(SerialPort self,dlt645_t *ctx, uint32_t code, uint8_t *read_data, dlt645_protocal protocal);
//十进制转BCD码(32位)
