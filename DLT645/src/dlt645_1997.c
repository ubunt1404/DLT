 ...
 */
int dlt645_1997_read_data(SerialPort self,
						  dlt645_t *ctx,
                          uint32_t code,
                          uint8_t *read_data)
{
    ...
    if (dlt645_send_msg(self, ctx, send_buf, DL645_1997_RD_CMD_LEN) < 0)
    {
        DLT645_LOG("send data error!\n");
        return -1;
    }

    if (dlt645_receive_msg(self, ctx, read_buf, DL645_RESP_LEN, code, DLT645_1997) < 0)
    {
